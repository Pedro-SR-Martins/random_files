...
server <- function(input, output) {
  ...
  output$download_btn <- downloadHandler(
    filename = function() {
      if (input$output_choice == "Bootstrapped Correlations") {
        return("bootstrapped_correlations.csv")
      } else {
        return("correlation_matrix.csv")
      }
    },
    content = function(file) {
      output_data_df <- output_data()
      write.csv(output_data_df, file, row.names = FALSE)
    }
  )
}
